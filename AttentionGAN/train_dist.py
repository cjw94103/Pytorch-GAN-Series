...
def main_worker(rank, world_size):
    ...
    D_A = NLayerDiscriminator(input_nc=args.channels, ndf=args.ndf, n_layers=args.n_layers, norm_layer=torch.nn.InstanceNorm2d).to(rank)
    D_A = torch.nn.parallel.DistributedDataParallel(D_A, device_ids=[rank])
    
    D_B = NLayerDiscriminator(input_nc=args.channels, ndf=args.ndf, n_layers=args.n_layers, norm_layer=torch.nn.InstanceNorm2d).to(rank)
